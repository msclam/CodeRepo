...
def getGaiaList(filename):
    with open(filename, mode='r') as file:
        ...
        while True:
            ...
            gaia_info = model.GaiaStarInformation(content[0], float(content[1]), float(content[2]), float(content[3]),
        ...
        while flag:
            ...
            for i in range(4):
                ...
                if i == 0:
                    ...
                    referstar = model.ReferStarInformation(float(content[0]), float(content[1]), float(content[2]),
                                                           float(content[3]), 0, 0)
                    refer_list.append(referstar)
                refer_table[number] = refer_list
                # 非空才进行转换，否则可能读到最后一行空字符的情况
        # for key, value in refer_table.items():
        #     print(key, value)
        return refer_table
...
# gaia星表文件表格，用于匹配查找使用，返回一个 number -- filename字典，通过number找对应的名字
...
def readFile_1():
    with open('./file/InputStarXY/Un_YFwb050070.txt') as file:
        x = []
        y = []
