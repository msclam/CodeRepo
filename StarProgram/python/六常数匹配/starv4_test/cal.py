...
# 与gaiaStarList中的所有gaiaStar进行比较


# 根据传入的gaiaStar和referStar进行匹配
def judge(error, inputStarList, gaiaStarList, referStarList, filename):
    ...
    for i in range(len(gaiaStarList)):  # 因为匹配出多颗星，所以选择最小的误差的添加到输出信息中
        gaia = gaiaStarList[i]
        flag = False
        minError = float('inf')
        instar = []
        for k in range(len(inputStarList)):
            _instar = inputStarList[k]
            # 求解eN，eN = [e,n]
            eN = [0, 0]
            calculateStandardCoordinatesBy6CModel(_instar, mc6, eN)
            ...
            errorRA = math.fabs(ra - gaia.RA)
            errorDEC = math.fabs(de - gaia.DEC)
        ...
        if flag:
            ...
            print(model.StarInformation(gaia, instar))
            inputStarList.remove(instar)
    ...
    for i in range(len(inputStarList)):
        failStar = inputStarList[i]
        # 求解eN，eN = [e,n]
        eN = [0, 0]
        calculateStandardCoordinatesBy6CModel(failStar, mc6, eN)
        # 根据eN求解raDe, raDe = [ra,de]
        raDe = [0, 0]
        transformToSkyPosition(eN[0], eN[1], raDe)
        ra = raDe[0]
        de = raDe[1]
        failedList.append(model.FailedStar(failStar.x, failStar.y, ra, de))

    # filename Un_YFwb050070.txt
    deal.outputSuccessList(successList, './file/result/Matched/' + filename[3:-4] + '.txt')
    deal.outputFailedList(failedList, './file/result/Unmatched/Unmatched' + filename[-7:-4] + '.txt')
